...
class SliceView(pg.GraphicsLayoutWidget):
    ...
    sigDrawStarted = QtCore.pyqtSignal()
    sigDrawFinished = QtCore.pyqtSignal()
...
class MaskLegendButton(QtWidgets.QToolButton):
    ...
    contextMenuRequested = QtCore.pyqtSignal(int, object)
    ...
    def contextMenuEvent(self, event):
        self.contextMenuRequested.emit(self.index, event.globalPos())
        event.accept()


class ColorLegendWidget(QtWidgets.QFrame):
    sigMaskToggled = QtCore.pyqtSignal(int, bool)
    sigMaskContextMenu = QtCore.pyqtSignal(int, object)
    ...
    def set_items(self, items):
        ...
        for i, item in enumerate(items):
            ...
            button.contextMenuRequested.connect(self.sigMaskContextMenu)
...
class Visualization(QtWidgets.QMainWindow):
    def __init__(self, image_sets, case_identifiers=None, case_loader=None):
        ...
        self._fill_applied = False
        self._copied_mask_slice = None
        self._copied_mask_orientation = None

        self._build_ui()
        self._connect_events()
        self._init_views()
        self._init_mask_shortcuts()
        self._active_slice_view = self.axi_view
    ...
    def _build_ui(self):
        ...
        self.mask_help_label = QtWidgets.QLabel("Right-click a mask for editing tools")
        mask_layout.addWidget(self.mask_help_label)
        ...
        self.setStyleSheet(
            ...
            QLineEdit, QComboBox, QSpinBox {
                background-color: black; color: white; border: 1px solid #777; padding: 3px;
            }
            QMenu { background-color: black; color: white; border: 1px solid #777; }
            QMenu::item:selected { background-color: orange; color: black; }
            """
        )
    ...
    def _connect_events(self):
        ...
        for view in (self.sag_view, self.cor_view, self.axi_view):
            view.sigDrawStarted.connect(self._on_draw_started)
            view.sigDrawFinished.connect(self._on_draw_finished)
        ...
        self.legend_widget.sigMaskContextMenu.connect(self._show_mask_context_menu)
    ...
    def _show_mask_context_menu(self, index, global_pos):
        self.mask_selector.setCurrentIndex(index)
        self.active_mask_index = index
        menu = QtWidgets.QMenu(self)

        draw_action = menu.addAction("Draw")
        erase_action = menu.addAction("Erase")
        fill_action = menu.addAction("Fill bucket")
        for action in (draw_action, erase_action, fill_action):
            action.setCheckable(True)
        draw_action.setChecked(self.draw_button.isChecked() and not self.fill_mode)
        erase_action.setChecked(self.erase_button.isChecked())
        fill_action.setChecked(self.fill_mode)

        brush_menu = menu.addMenu(f"Brush size ({self.brush_size.value()} voxels)")
        for size in (1, 2, 3, 4, 5, 7, 10):
            action = brush_menu.addAction(str(size))
            action.setCheckable(True)
            action.setChecked(size == self.brush_size.value())
            action.triggered.connect(lambda _checked, value=size: self.brush_size.setValue(value))

        menu.addSeparator()
        copy_action = menu.addAction("Copy contour from this slice\tCtrl+C")
        paste_action = menu.addAction("Paste contour onto this slice\tCtrl+V")
        menu.addSeparator()
        color_action = menu.addAction("Choose mask color…")
        clear_action = menu.addAction("Clear mask")
        save_action = menu.addAction("Save mask as NIfTI…")

        draw_action.triggered.connect(lambda: self._select_edit_mode("draw"))
        erase_action.triggered.connect(lambda: self._select_edit_mode("erase"))
        fill_action.triggered.connect(lambda: self._select_edit_mode("fill"))
        copy_action.triggered.connect(self._copy_current_mask_slice)
        paste_action.triggered.connect(self._paste_mask_slice)
        color_action.triggered.connect(self._choose_mask_color)
        clear_action.triggered.connect(self._clear_active_mask)
        save_action.triggered.connect(self._save_active_mask)
        menu.exec_(global_pos)

    def _select_edit_mode(self, mode):
        self.fill_mode = mode == "fill"
        self.draw_button.setChecked(mode == "draw")
        self.erase_button.setChecked(mode == "erase")
        self._set_drawing_enabled(True)
        self.statusBar().showMessage(f"{mode.title()} mode — left-drag on a slice", 4000)
    ...
    def _drawing_plane_and_center(self, view, x_phys, y_phys, mask):
        """Return the edited 2-D mask plane and cursor voxel within it."""
    ...
    def _on_view_drawn(self, view, x_phys, y_phys):
        ...
        plane, center = self._drawing_plane_and_center(view, x_phys, y_phys, mask)
        if self.fill_mode:
            if self._fill_applied:
                return
        ...
        points = [center]
        if self._last_draw_point is not None and self._last_draw_point[0] is view:
            previous = self._last_draw_point[1]
            distance = max(abs(center[0] - previous[0]), abs(center[1] - previous[1]))
            if distance:
                points = zip(
                    np.linspace(previous[0], center[0], distance + 1),
                    np.linspace(previous[1], center[1], distance + 1),
                )
        value = 0 if self.erase_button.isChecked() else 1
    ...
    def _fill_connected_area(plane, center):
        """Fill the 4-connected region containing ``center`` on one slice."""
        target = int(plane[center])
        replacement = 1 - target
        pending = [center]
        plane[center] = replacement
        while pending:
            row, column = pending.pop()
            for next_row, next_column in ((row - 1, column), (row + 1, column), (row, column - 1), (row, column + 1)):
                if (
                    0 <= next_row < plane.shape[0]
                    and 0 <= next_column < plane.shape[1]
                    and plane[next_row, next_column] == target
                ):
                    plane[next_row, next_column] = replacement
    ...
    def keyPressEvent(self, ev):
        if ev.matches(QtGui.QKeySequence.Copy):
            self._copy_current_mask_slice()
            ev.accept()
            return
        if ev.matches(QtGui.QKeySequence.Paste):
            self._paste_mask_slice()
            ev.accept()
            return
        if ev.key() in (QtCore.Qt.Key_Right, QtCore.Qt.Key_Up):
            self._step_active_slice(1)
            ev.accept()
            return
        if ev.key() in (QtCore.Qt.Key_Left, QtCore.Qt.Key_Down):
            self._step_active_slice(-1)
            ev.accept()
            return
    ...
    def _init_mask_shortcuts(self):
        """Install window-wide shortcuts so slice widgets cannot consume copy/paste."""
        self.copy_contour_shortcut = QtWidgets.QShortcut(QtGui.QKeySequence.Copy, self)
        self.paste_contour_shortcut = QtWidgets.QShortcut(QtGui.QKeySequence.Paste, self)
        self.copy_contour_shortcut.setContext(QtCore.Qt.WindowShortcut)
        self.paste_contour_shortcut.setContext(QtCore.Qt.WindowShortcut)
        self.copy_contour_shortcut.activated.connect(self._copy_current_mask_slice)
        self.paste_contour_shortcut.activated.connect(self._paste_mask_slice)

    def _step_active_slice(self, step):
        """Move one slice in the most recently used view."""
        view = self._active_slice_view or self.axi_view
        self._on_view_scrolled(view, step)

    def _current_mask_slice(self):
        """Return the active mask slice and its anatomical orientation."""
        mask = self._editable_mask()
        if mask is None:
            return None, None
        view = self._active_slice_view or self.axi_view
        if view is self.sag_view:
            return mask["data"][:, :, self.current_sagittal], "sagittal"
        if view is self.cor_view:
            return mask["data"][:, self.current_coronal, :], "coronal"
        return mask["data"][self.current_axial, :, :], "axial"

    def _copy_current_mask_slice(self):
        mask_slice, orientation = self._current_mask_slice()
        if mask_slice is None:
            self.statusBar().showMessage("Select a mask before copying a contour.", 4000)
            return
        self._copied_mask_slice = np.array(mask_slice, dtype=np.uint8, copy=True)
        self._copied_mask_orientation = orientation
        self.statusBar().showMessage(f"Copied {orientation} contour. Move to another slice and press Ctrl+V.", 5000)

    def _paste_mask_slice(self):
        destination, orientation = self._current_mask_slice()
        if destination is None:
            self.statusBar().showMessage("Select a mask before pasting a contour.", 4000)
            return
        if self._copied_mask_slice is None:
            self.statusBar().showMessage("No copied contour. Press Ctrl+C on a mask slice first.", 4000)
            return
        if orientation != self._copied_mask_orientation or destination.shape != self._copied_mask_slice.shape:
            self.statusBar().showMessage("Paste into another slice of the same view orientation.", 5000)
            return
        destination[...] = self._copied_mask_slice
        self._update_all_views()
        self.statusBar().showMessage(f"Pasted contour into the current {orientation} slice.", 4000)
