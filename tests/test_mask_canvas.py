...
def test_window_shortcuts_copy_paste_when_slice_has_focus(viewer, monkeypatch):
    monkeypatch.setattr(QInputDialog, "getText", lambda *args: ("target", True))
    viewer._create_mask()
    viewer._active_slice_view = viewer.axi_view
    viewer.axi_view.setFocus()
    source_slice = viewer.current_axial
    viewer.masks[0]["data"][source_slice, 1, 1] = 1

    viewer.copy_contour_shortcut.activated.emit()
    viewer._step_active_slice(1)
    viewer.paste_contour_shortcut.activated.emit()

    assert viewer.masks[0]["data"][viewer.current_axial, 1, 1] == 1
